...
```json
{
  "authorizationCode": "4/0AfJohX....",
  "redirectUri": "https://frontend.example.com/auth/google/callback"
}
```
| Field             | Type   | Required | Description                                          |
| ----------------- | ------ | -------- | ---------------------------------------------------- |
| authorizationCode | String | O        | 구글 OAuth 로그인 성공 후 프론트가 받은 인가 코드(authorization code)  |
...
| X-Refresh-Token  | {refreshToken}             | JWT Refresh Token     |
...
| ---------- | ------- | -------- | ------------------- |
| id         | Long    | O        | 내부 사용자 ID           |
| email      | String  | O        | 한세대학교 이메일(@hansei.ac.kr) |
| name       | String  | X        | 사용자 이름              |
| department | String  | X        | 학과                  |
| isNew      | Boolean | O        | 신규 가입 여부            |
...
| Key              | Value            | Required | Description       |
| ---------------- | ---------------- | -------- | ----------------- |
| X-Refresh-Token  | {refreshToken}   | O        | JWT Refresh Token |
...
| Key             | Value          | Required | Description           |
| --------------- | -------------- | -------- | --------------------- |
| X-Refresh-Token | {refreshToken} | O        | 폐기할 JWT Refresh Token |
